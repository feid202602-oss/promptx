...
3. 如需处理需求 PDF，可直接把单栏 PDF 拖入编辑页，系统会尝试按图文顺序解析。
4. 自动保存完成后，复制公开页链接或 Raw 文本链接给模型。
5. 如需继续编辑，直接打开对应文档的编辑页即可。
...
- 当前只支持非扫描件 PDF
- 当前优先支持单栏、简单文本 + 图片的需求文档
- 解析结果会直接变成普通文本块和图片块，插入当前编辑位置
- 暂不保证多栏排版、复杂表格、宣传页式 PDF 的图文顺序完全准确
